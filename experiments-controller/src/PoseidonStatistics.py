...
class PoseidonWorkflowStatistics:
    dag_file = None
    workflow_name = None
    braindump_file = None
    jobstate_log = None
    ...
    #        print(node, level)
    #        for job in self.dag[node]["childNodes"]:
        ...
        for transfer_event in transfer_events:
            if transfer_event["dag_job_id"].startswith("stage_in"):
                if (self.dag[transfer_event["dag_job_id"]]["timeline"]["stage_in_start"] is None):
                    ...
                    #self.dag[transfer_event["dag_job_id"]]["stage_in_rate"] = int(transfer_event["total_bytes"])
                    #self.dag[transfer_event["dag_job_id"]]["stage_in_source"] = int(transfer_event["total_bytes"])
                else:
                ...
                if (self.dag[transfer_event["dag_job_id"]]["timeline"]["stage_out_start"] is None):
                    ...
                    #self.dag[transfer_event["dag_job_id"]]["stage_out_rate"] = int(transfer_event["total_bytes"])
                    #self.dag[transfer_event["dag_job_id"]]["stage_out_source"] = int(transfer_event["total_bytes"])
                else:
    ...
    def retrieve_kickstart_from_elasticsearch(self):
        ...
        for job in self.dag:
            ...
            #print(json.dumps(self.dag[job]["kickstart_data"], indent=2))
    ...
    def populate_subworkflow_statistics(self):
        for job in self.dag:
            if job.startswith("pegasus-plan"):
                ...
                run_dir = os.path.join(self.dag[job]["subwf"]["dir"], self.dag[job]["subwf"]["relative_submit_dir"])
                ...
                self.dag[job]["subwf"]["subwf_statistics"] = PoseidonWorkflowStatistics(self.dag[job]["subwf"]["basename"], dag_file, braindump_file, jobstate_log)
    ...
    def prepare_pandas_json(self, anomaly_type, anomaly_nodes):
        ...
        for job in self.dag:
            ...
            #temp_dict["kickstart_executables_cpu_time"] = ";".join(map(str, dag[job]["kickstart_data"]["cpu_time"]))
            #temp_dict["kickstart_executables_exitcode"] = ";".join(map(str, dag[job]["kickstart_data"]["exitcode"]))
...
class PoseidonStatistics:
    ...
    def __init__(self, experiments_logfile, workflows_config):
        self.logger = logging.getLogger("poseidon")
        self.experiments_log = load_experiments_log(experiments_logfile)
        self.workflow_list = load_workflow_list(workflows_config)
        return
    ...
    def generate(self):
        ...
        for workflow_name in self.workflow_list:
            ...
            workflow_submit_dir = os.path.join(workflow_submit_dir, os.getlogin(), "pegasus", workflow_name)
            ...
            for sub_folder in os.listdir(workflow_submit_dir):
                ...
                workflow_statistics = PoseidonWorkflowStatistics(workflow_name, dag_file, braindump_file, jobstate_log)
                ...
                #workflow_statistics.print_adjacency_list()
                #exit()
